...
function VoterListCtrl($location, pollFactory,
    ...
    voterListCtrl.result = function(index, poll) { //when "result" is clicked

        let choice = null;

        if ($sessionStorage.savedPoll != null)
        ...
        pollFactory.getVote($sessionStorage.user._id, poll._id)
            .success(function(vote) {
                $sessionStorage.savedPoll.choice = null;

                if (vote != null)
                    $sessionStorage.savedPoll.choice = vote.choice;
...
function ResultCtrl($sessionStorage, socket, pollFactory) {
    var resultCtrl = this;
    ...
    socket.on('server:updateChoices', function(data) {
        //check if user is presently viewing this result so that you can update
        //the result without changing the page
        if ($sessionStorage.savedPoll.poll._id == data.poll._id)
